  ...
  <div class="verify-task">
    ...
    <el-card class="list-card">
      <el-table v-loading="loading" :data="taskList" style="width: 100%">
        ...
        <el-table-column prop="collectorName" label="核查人" width="100" />
...
import { getVerifyTaskList } from '@/api/task'
...
async function loadTaskList() {
  loading.value = true
  try {
    const res = await getVerifyTaskList({ pageNum: pageNum.value, pageSize: pageSize.value, ...searchForm })
...
function handleExecute(row) {
  router.push(`/task/verify/${id}?action=execute`)
}
</script>
